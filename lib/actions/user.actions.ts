...
// import { getUserInfo } from './../actions_(success)/user.actions';
...
export const exchangePublicToken = async ({
  ...
  try {
    ...
    // If the funding source using URL is not created, throw an error
    if (!fundingSourceUrl) throw Error;

    // Create a bank using the user ID, item ID, account ID, access token, funding source URL, and sharable ID
    ...
    console.error(
      "An error occurred while creating exchangting token: ",
      error
    );
  }
};
